...
// Base62: 0-9 (10), A-Z (26), a-z (26) = 62 caracteres
// N0 → N1 → N2 ... → N9 → NA → NB → NZ → Na → Nb → Nz
//--------------------------------------------------
...
//--------------------------------------------------
// BUSCA OU CRIA TOKEN (VERSÃO OTIMIZADA)
//--------------------------------------------------

async function obterOuCriarToken(tabela, prefixo, valor, client, contador) {
  ...
  const token = tokenProgressivo(prefixo, contador);
...
//--------------------------------------------------
// POPULAR (VERSÃO CORRIGIDA - TOKENS PROGRESSIVOS)
//--------------------------------------------------
...
async function popular() {
  ...
  // Contadores para tokens progressivos
  let contadorNomes = 0;
  let contadorSobrenomes = 0;
  let contadorRuas = 0;
  ...
  for (let batch = 0; batch < TOTAL_REGISTROS / BATCH_SIZE; batch++) {
    ...
    try {
      ...
      for (let i = 0; i < BATCH_SIZE; i++) {
        ...
        // Inserir na tabela normal (para referência)
        ...
        //--------------------------------------------------
        // TOKENIZAÇÃO MIR COM CONTADORES PROGRESSIVOS
        //--------------------------------------------------
        
        // Nome: token progressivo N0, N1, N2...
        const nomeToken = await obterOuCriarToken('lexical_nome', 'N', nome, client, contadorNomes);
        if (nomeToken && nomeToken !== `N${encodeBase62(contadorNomes)}`) {
          // Token já existia, não incrementa
        } else if (nomeToken) {
          contadorNomes++;
        }
        
        // Sobrenome: token progressivo S0, S1, S2...
        // Para sobrenome composto, cada parte recebe seu próprio token
        const partesSobrenome = sobrenome.split(' ');
        const tokensSobrenome = [];
        
        for (const parte of partesSobrenome) {
          const token = await obterOuCriarToken('lexical_sobrenome', 'S', parte, client, contadorSobrenomes);
          tokensSobrenome.push(token);
          if (token === `S${encodeBase62(contadorSobrenomes)}`) {
            contadorSobrenomes++;
          }
        }
        const sobrenomeToken = tokensSobrenome.join('|');
        
        // Rua: token progressivo R0, R1, R2...
        const ruaToken = await obterOuCriarToken('lexical_rua', 'R', rua, client, contadorRuas);
        if (ruaToken === `R${encodeBase62(contadorRuas)}`) {
          contadorRuas++;
        }
        
        // Cidade: token progressivo C0, C1, C2...
        const cidadeToken = await obterOuCriarToken('lexical_cidade', 'C', cidade, client, contadorCidades);
        if (cidadeToken === `C${encodeBase62(contadorCidades)}`) {
          contadorCidades++;
        }
        
        // CEP: token progressivo E0, E1, E2... (E de Endereço)
        const cepToken = await obterOuCriarToken('lexical_cep', 'E', cep, client, contadorCeps);
        if (cepToken === `E${encodeBase62(contadorCeps)}`) {
          contadorCeps++;
        }
        
        // CPF MNE: 9 primeiros dígitos em Base62
        const cpfBase = cpf.substring(0, 9);
        const cpfCompactado = encodeBase62(parseInt(cpfBase));
        ...
        if (index % 5000 === 0) {
          ...
          console.log(`   Tokens: N=${contadorNomes} S=${contadorSobrenomes} R=${contadorRuas} C=${contadorCidades} E=${contadorCeps}`);
        }
      }
...
async function exibirEstatisticas() {
  ...
  try {
    ...
    console.log(`📝 Registros Normal: ${parseInt(totalNormal.rows[0].count).toLocaleString()}`);
    console.log(`🔐 Registros MIR: ${parseInt(totalMir.rows[0].count).toLocaleString()}`);
    console.log(`📚 Tokens Nomes: ${parseInt(totalNomes.rows[0].count).toLocaleString()}`);
    console.log(`📚 Tokens Sobrenomes: ${parseInt(totalSobrenomes.rows[0].count).toLocaleString()}`);
    console.log(`📚 Tokens Ruas: ${parseInt(totalRuas.rows[0].count).toLocaleString()}`);
    console.log(`📚 Tokens Cidades: ${parseInt(totalCidades.rows[0].count).toLocaleString()}`);
    console.log(`📚 Tokens CEPs: ${parseInt(totalCeps.rows[0].count).toLocaleString()}`);
    
    // Mostrar exemplos de tokens
    console.log('\n📌 EXEMPLOS DE TOKENS PROGRESSIVOS:');
    
    const exemplosNomes = await pool.query('SELECT token, valor FROM lexical_nome LIMIT 3');
    ...
    const exemplosSobrenomes = await pool.query('SELECT token, valor FROM lexical_sobrenome LIMIT 3');
    ...
    const exemplosRuas = await pool.query('SELECT token, valor FROM lexical_rua LIMIT 3');
    if (exemplosRuas.rows.length > 0) {
      console.log('   🏠 Ruas:');
      exemplosRuas.rows.forEach(ex => {
        console.log(`      ${ex.token} → "${ex.valor}"`);
      });
    }
    
    // Calcular economia (simplificado)
